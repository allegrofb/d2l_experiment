...
print(output.shape, state.shape)  # torch.Size([4, 7, 10]) torch.Size([2, 4, 16])


#@save
def sequence_mask(X, valid_len, value=0):
    """Mask irrelevant entries in sequences."""
    maxlen = X.size(1)  # X.size()[1]
...
def train_seq2seq(net, data_iter, lr, num_epochs, tgt_vocab, device):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in data_iter:
            ...
            # print(X.size(),X_valid_len,Y.size(),Y_valid_len)
            # X = (64, 10), sometimes have (25, 10)
            # X_valid_len, (64,)
...
print(f'len(src_vocab)={len(src_vocab)},len(tgt_vocab)={len(tgt_vocab)}')
